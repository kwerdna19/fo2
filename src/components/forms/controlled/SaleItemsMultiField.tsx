...
type Items = z.infer<typeof saleItemsSchema>

export default function SaleItemsMultiField({ className, items }: { className?: string, items: Pick<Item, 'id' | 'name' | 'spriteUrl'>[] }) {
          ...
          return <div className="grid grid-cols-4 gap-x-6" key={i}>
            ...
            <Input
              ...
              onChange={e => field.onChange((field.value ?? []).map((el, index) => {
                if(i === index)
                  return Object.assign({}, el, { price: parseInt(e.target.value) })
                return el
              }))}
            />
            </div>
