...
export default function FormCheckbox({ className, field, label }: { className?: string, field: FieldMetadata<boolean>, label: string }) {

  const errMessage = field.errors?.at(0)

  return (<div className={cn("space-y-1", className)}>
      <FieldLabel field={field}>{label}</FieldLabel>
      <Checkbox className="block" {...getInputProps(field, { type: 'checkbox' })} key={field.key} type={undefined} />
