...
export function NavMenu({ className, areas, mobile }: { className?: string, areas: Areas, mobile?: boolean }) {
  ...
  const pathMatches = (str: string) => path === str
	...
	return (
		...
		<NavigationMenuList className={cn(mobile && "flex flex-col gap-4 w-full")}>
			...
			<NavigationMenuItem className={itemClassName}>
				<NavigationMenuLink active={pathMatches('/items')} asChild className={linkClassName}>
			...
			<NavigationMenuItem className={itemClassName}>
				<NavigationMenuLink active={pathMatches('/npcs')} asChild className={linkClassName}>
