...
export const columns = [
	columnHelper.accessor("level", {
		id: "level",
		cell: ({ getValue }) => (
			<div className="flex justify-center">{getValue()}</div>
		),
	...
	columnHelper.accessor((row) => (row.goldMin + row.goldMax) / 2, {
		...
		cell: ({ row }) => (
			<DropGold
				...
				goldMin={row.original.goldMin}
				goldMax={row.original.goldMax}
			/>
		),
		header: ({ column }) => {
			return (
				<div className="flex justify-center">
					<SortButton column={column}>Gold</SortButton>
				</div>
			);
		},
	}),
	columnHelper.accessor("health", {
		...
		header: ({ column }) => {
			return (
				<div className="flex justify-center">
					<SortButton column={column}>Health</SortButton>
				</div>
			);
		},
	}),
	columnHelper.accessor((row) => row.drops.map((d) => d.item.name).join(", "), {
		...
		header: "Loot",
		cell: ({ row }) => (
			<DropsList drops={row.original.drops} className="flex-nowrap" />
		),
	}),
];
			...
			<div className="flex items-center mb-4 gap-8">
				...
				<div className="flex items-center space-x-2">
					<Checkbox
						...
						onCheckedChange={(c) =>
							table.getColumn("boss")?.setFilterValue(Boolean(c))
						}
					/>
					<label
						htmlFor="bosses"
						...
						{table.getHeaderGroups().map((headerGroup) => (
							<TableRow key={headerGroup.id}>
								{headerGroup.headers.map((header) => {
									...
									return (
										<TableHead
											colSpan={header.id === "level" ? 2 : undefined}
											key={header.id}
										>
