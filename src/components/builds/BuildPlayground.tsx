...
import { type Build, STAT_POINTS_PER_LEVEL, Slot, playerSlots, getAllStats, LEVEL_CAP, type Stat, BASE_STATS, BASE_BASIC_STAT, stats } from "~/utils/fo"
...
export default function BuildPlayground({ items }: { items: Item[] }) {
  ...
  const [selectedItemIndices, setSelectedItemIndices] = useState<Record<Slot, number | null>>({
    [Slot.HEAD]: null,
    [Slot.FACE]: null,
    [Slot.BACK]: null,
    [Slot.SHOULDERS]: null,
  ...
  const updateSlot = (slot: Slot, newItemId: string | null | undefined) => {
    const index = newItemId ? items.findIndex(item => newItemId === item.id) : null
  ...
  const renderStatPointField = (stat: Stat) => {
    ...
    return (<div className="flex items-center gap-x-4">
      <div className="text-md flex-1 px-1 font-bold uppercase">
        {stat}
      </div>
      <Input
      ...
      />
      <Button size="icon" className="flex-shrink-0" onClick={() => {
        setValue(`statPoints.${stat}`, BASE_BASIC_STAT)
      }}>
          <ChevronsDown className="h-5 w-5" />
      </Button>
      <div className="flex flex-shrink-0 gap-x-2">
        <Button size="icon" onClick={() => setValue(`statPoints.${stat}`, statAssignments[stat]-1)} >
  ...
  return (<div className="w-full flex flex-col-reverse lg:flex-row gap-6 lg:gap-3">
    <Card className="flex-1">
        ...
        <fieldset className="p-3 space-y-1">
          ...
          <div className="space-y-4">
            ...
            <div>
              Remaining: <span className={cn(statPointsRemaining < 0 && "text-red-500")}>{statPointsRemaining}</span>
            </div>
          </div>
      ...
      <Card className="mt-3 p-2 flex gap-8">
        <div className="space-y-1 flex-1">
          {(['sta', 'str', 'agi', 'int', 'armor'] as const).map(s => {
            return <div key={s} className={cn("flex justify-between", s === 'armor' ? "capitalize" : "uppercase")}>
              <div>{s}</div>
              <div>{allStats[s]}</div>
            </div>
          })}
        ...
        <div className="space-y-1 flex-1">
          {(['atkPower', 'crit', 'dodge', 'atkSpeed'] as const).map(s => {
            return <div key={s} className="flex justify-between capitalize">
              <div>{s}</div>
              <div>{Math.round(allStats[s]*100)/100}{['crit', 'dodge'].includes(s) ? ' %' : ''}</div>
            </div>
          })}
