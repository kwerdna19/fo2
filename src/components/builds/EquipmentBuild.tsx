...
export default function EquipmentBuild({ possibleBuilds, selectedBuild, stat, updateSlot, energy, health }: Props) {
  ...
  return (<div className="grid grid-cols-[1fr_400px_1fr] gap-4 grid-rows-7">
      {playerSlots.map((slot, i) => {
        ...
        return (<div key={slot} style={{gridRow: row , gridColumn: col }}>
       <BuildItem slot={slot} item={item} tooltipSide={col === 1 ? 'left' : 'right'} stat={stat} switchOptions={hasOptions ? (inc: 1 | -1) => updateSlot(slot, inc) : undefined}/>
      </div>)
      }
      )}
