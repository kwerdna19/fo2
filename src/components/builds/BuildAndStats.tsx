...
export default function BuildAndStats({ items, stat, opt, level }: { items: Item[], stat?: BasicStats, opt: 'max' | 'min', level: number }) {
  ...
  const allStats = getAllStats(build, level, stat && {
    ...
    <div className="border mt-3 p-2 rounded-sm flex gap-8">
      ...
      <div className="space-y-1 flex-1">
        {(['sta', 'str', 'agi', 'int', 'armor'] as const).map(s => {
          return <div key={s} className={cn("flex justify-between", s === stat && "font-bold")}>
      ...
      <div className="space-y-1 flex-1">
        {(['atkPower', 'crit', 'dodge', 'atkSpeed'] as const).map(s => {
          return <div key={s} className={cn("flex justify-between")}>
            <div>{s[0]?.toUpperCase()}{s.slice(1)}</div>
