...
function MobField({
	mobs,
	field,
}: { mobs: Props["mobs"]; field: FieldMetadata<string> }) {
	const control = useInputControl(field);
	const [open, setOpen] = useState(false);

	const [selectedItem, setSelectedItem] = useState(
		mobs.find((mob) => mob.id === field.value),
	);
...
export default function DroppedByMobsMultiField({
	className,
	mobs,
	field,
	label,
					...
					return (
						...
						>
							<div className="grid grid-cols-4 col-span-3 gap-x-3">
								<MobField field={mobId} mobs={mobs} />
								<Input
