...
import type { EquippableType, Prisma } from "@prisma/client";
...
import { itemSearchFilterSchema, itemSearchParamCache } from "./search-params";

const requiredFields = schema.definitions.Mob.required;
const searchFields = ["name", "desc", "slug"];

export const getAllItems = async (searchParams: SearchParams) => {
	const input = itemSearchFilterSchema
	...
	// if (typeof equipTypes === "object" && input.equipTypes) {
	// 	conditions.push({
	// 		equip: {
